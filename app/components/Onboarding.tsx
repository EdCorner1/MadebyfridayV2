...
export default function Onboarding({ onComplete }: { onComplete: (profile: Profile) => void }) {
  ...
  const handleComplete = () => {
    localStorage.setItem('friday_profile', JSON.stringify(profile));
    onComplete(profile);
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#FDFCF7]">
      <div className="w-full max-w-md rounded-3xl border border-[#ece7df] bg-white p-8 shadow-xl">
        ...
        <div className="mt-8">
          ...
          <button
            onClick={handleComplete}
            className="mt-2 w-full text-center text-sm text-[#aaa] hover:text-[#888] transition-colors"
          >
            Skip for now
          </button>
        </div>
